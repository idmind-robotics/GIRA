...
class HW:
    def __init__(self):
        ...
        self.shakeDetector = imu.shakeDetection(window_size = 5, threshold = 1.05)
        ...
        self.values={"OnDock": False, "BAT_TEMP": -1, "IsLocked": False,
                    ...
                    "BAT_VOLT": 0, "MotorCurrent": 0, "MotorPower": 0, "MotorEnabled": False}
    ...
    def read(self, GNStime=time.localtime()):
        ...
        self.values["BAT_TEMP"] = self.termistorConversion(self.TSENSE)
        ...
        self.values["BAT_VOLT"], self.values["MotorCurrent"], self.values["MotorPower"] = self.motorSensor.read()
        ...
        if self.values["ChargerVoltage"] >= 30 and self.values["OnDock"] is False: # Charger Detected - Docked: Charger Voltage is expected to be 42 Volt
            ...
            self.turnOffCharger()
            self.values["BAT_VOLT"], self.values["MotorCurrent"], self.values["MotorPower"] = self.motorSensor.read()      
            if self.values["BAT_VOLT"] > self.CutOffVoltage:
                self.forceMotorOn()
                self.turnOn5V()
            
        if self.values["OnDock"] is True and self.values["BAT_VOLT"] >= 41.7 and self.values["ChargerEnabled"]:
            self.turnOffCharger()
            
        elif self.values["OnDock"] is True and self.values["BAT_VOLT"] <= 41.3 and not self.values["ChargerEnabled"]:
            self.turnOnCharger()
        ...
        '''
        if self.values["BAT_VOLT"] <= self.CutOffVoltage and self.values["OnDock"] is False and self.batteryAlarmFlag is False: # Low Battery
            self.buzzer.alarm = True
        ...
        if (self.values["BAT_VOLT"] > 34 or self.values["OnDock"] is True) and self.batteryAlarmFlag is True:
            self.buzzer.alarm = False
            self.batteryAlarmFlag = False
            self.LowBattery = False
