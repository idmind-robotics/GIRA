...
class Modem(object):
    ...
    def __init__(self, uart=None, MODEM_PWKEY_PIN=None, MODEM_RST_PIN=None, MODEM_POWER_ON_PIN=None, MODEM_TX_PIN=None, MODEM_RX_PIN=None):
        ...
        #These Variables are used to pass a http request parameters into the threaded communication loop
        self.url = ''
        self.mode = 'POST'
        self.data = ''
    ...
    def initialize(self):
        ...
        if not self.uart:
            ...
            # Setup UART
            self.uart = UART(2, 9600, timeout=1000, tx=self.MODEM_TX_PIN, rx=self.MODEM_RX_PIN)

        # Test AT commands
        retries = 0
        while True:
            try:
                self.modem_info = self.execute_at_command('imei') #modeminfo
            except:
                retries+=1
                if retries < 3:
                    print('A verificar ligação ao Modem... Tentativa(#{})'.format(retries))
                    time.sleep(3)
                else:
                    raise Exception("Erro na ligação, a tentar outra vez... Verificar se Módulo está alimentado / Conexões")
            else:
                break

        print('Ok, o Modem "{}" foi encontrado e está pronto a aceitar comandos'.format(self.modem_info))
        self.uart.read()
        ...
        self.ssl_available = True #self.execute_at_command('checkssl') == '+CIPSSL: (0-1)'
        time.sleep(1)
    ...
    def setupGSM(self):
        ...
        #time.sleep(5)
        self.uart.read()
        #self.execute_at_command('debug')
        #self.connect('net2.vodafone.pt', user='vodafone',pwd='vodafone') # Vodafone SIM
        print("A verificar ligação ao SIM")
        
        #self.connect(apn, user=usr, pwd=password)
        self.connect('umts', user='', pwd='') # NOS Sim
        time.sleep(2)
        self.uart.read()
        self.execute_at_command('setbusy')
        print("A tentar obter IP para conectar à internet...")
        ...
        print("Conexão à internet estabelecida corretamente")
        return True
    ...
    def execute_at_command(self, command, data=None, clean_output=True):
        ...
        # Dictionary containing different possible commands
        commands = {
                    'modeminfo':  {'string':'AT', 'timeout':3, 'end': 'OK'},
                    'debug':      {'string':'AT+CMEE=1', 'timeout':3, 'end': 'OK'},
                    'reg':	  	  {'string':'AT+CREG=1', 'timeout':3, 'end': 'OK'},  
                    ...
                    #'setapn':     {'string': 'AT+CSTT=,"{}"'.format(data), 'timeout':3, 'end': 'OK'},
                    ...
                    'opengprs':   {'string':'AT+SAPBR=1,1', 'timeout':6, 'end': 'OK'},
        ...
        while True:
            ...
            if not line:
                time.sleep_ms(500)
                empty_reads += 1
                if empty_reads > timeout*3:
                    print("Comando AT sem resposta")
                    #raise Exception('Sem resposta para o comando "{}" (timeout={})'.format(command, timeout))
                ...
                if len(line_str) >=3:
                    print('Resposta do Modem: {}'.format(line_str))
                    # Do we have an error?
                    if line_str == 'ERROR\r\n':
                        raise Exception("Erro de Comunicação Genérico")
                        break
    ...
    def get_ip_addr(self):
            ...
            pieces = output.split(',')
            if len(pieces) != 3:
                raise Exception('"{}" não é um endereço de IP válido'.format(output))
    ...
    def connect(self, apn, user='', pwd=''):
        if not self.initialized:
            raise Exception('Modem não inicializado, impossível conectar')
        ...
        while True:
            ...
            if ip_addr == "0.0.0.0":
                ...
                if retries > max_retries:
                    raise Exception('Impossível estabelecer conexão sem obter um endereço de IP Válido')
                print('A tentar obter endereço de IP Válido... (#')
                time.sleep(2)
            else:
                break
    ...
    def http_request(self, url, mode='POST', data=None, content_type='application/json'):
        # Protocol check.
        ...
        if not self.get_ip_addr():
            raise Exception('Modem não está conectado: Não temos endereço de IP válido')
        self.uart.read()
        ...
        if mode == 'GET':
            ...
            # start = time.ticks_ms()
            print(f"A enviar dados para o servidor: {data}") 
            ...
            raise Exception('Modo HTTP desconhecido "{}"'.format(mode))
    ...
    def getGpsStatus(self):
        ...
        rsp = response.split(': ')
        if rsp[1] == "Location Unknown":
            self.values["GPSFix"] = "GPS Error"
        else:
    ...
    def getGpsData(self):
        self.uart.read()
        if self.values["GPSFix"] == "Location 3D Fix":
