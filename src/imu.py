...
class IMU:
    ...
    **Quickstart: Importing and using the device**
    ...
    def __init__(self, i2c, address: int = 0x4C) -> None:
        ...
        self._mode = NORMAL
        self.xOFF = -0.04 # This values must be checked and changed for every IMU (OFFSET)
        self.yOFF = 0
        self.zOFF = 0.4
        self.previousx = 0
        self.previousy = 0
...
class shakeDetection:
    def __init__(self, window_size = 5, threshold = 1.03):  #window_size determines for how long the shaking has to occur and threshold determines how strong it has to be
    ...
    def detect(self, accx, accy, accz):
        ...
        if (self.avgAcc >= self.shake_threshold) or self.totalAcc >= self.violent_threshold:
            return True
        else:
            return False
...
class tiltDetection:
    ...
    def __init__(self, tilt_threshold = 70, time_threshold = 5): # Define for how long the bike must be tilted for Detection
        ...
        self.restingY = 0.7799  # These are the approximate measurements from an upright bike
...
mc3479 = IMU(i2c)
shakeDetector = shakeDetection(window_size = 15, threshold = 1.3)
while True:
    accx, accy, accz = mc3479.acceleration
