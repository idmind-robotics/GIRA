...
class PowerSensor:
    ...
    def sendCustomerCode(self):
        ...
        if binary_representation[-1] == "1":
            ...
            print("Customer code not accepted")
    ...
    def read(self):
        ...
        self.power = float(signed_unsigned_pinstant)
...
if len(devices) == 2:
    ...
    while True:
        ...
        '''
        if time.time() - enable_time > 5:
            controlval = not controlval
            enable_time = time.time()
            if controlval is False:
                print("Turning OFF MOSFET")
                controlout.value(0)
            else:
                print("TURNING ON MOSFET")
                controlout.value(1)
        '''
